...
test("Shift-click toggles selection and plain drag creates a full-containment marquee", async ({ page }) => {
...
test("right-clicking a selected asset opens bulk actions without triggering pane paste", async ({ page }) => {
  await seedDiagram(page)
  await selectNodes(page, ["a", "b"])
  await page.keyboard.press("Control+c")

  await node(page, "a").click({ button: "right" })
  const menu = page.getByRole("menu", { name: "Bulk selection actions" })
